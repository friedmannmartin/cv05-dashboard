<?php
  ...
  if (!empty($_POST)){
    ...
    if (empty($errors)){
      if(!empty($_POST['post_id'])){
        $updateQuery=$db->prepare('UPDATE posts SET category_id=:category, text=:text WHERE post_id=:post_id');
        $updateQuery->execute([
          ':category'=>$_POST['category'],
          ':text'=>$text,
  ...
  if(!empty($_GET['post_id'])){
    $editQuery=$db->prepare('SELECT * FROM posts WHERE post_id=:post_id LIMIT 1');
    $editQuery->execute([
      ':post_id'=>$_GET['post_id']
    ]);
    $editedPost=$editQuery->fetch(PDO::FETCH_ASSOC);
    $_POST['category']=$editedPost['category_id'];
    $_POST['text']=$editedPost['text'];
  }
